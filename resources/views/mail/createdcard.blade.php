<h1>Tarjeta creada: {{ $data['name'] }} {{ $data['surname1'] }}</h1>
<h2>Se ha creado una nueva <b>tarjeta de doctor</b>: {{ $root }}/es/card?id={{ $id }}</h2>
...
<h1>Card created: {{ $data['name'] }} {{ $data['surname1'] }}</h1>
<h2>A new <b>doctor card</b> has been created: {{ $root }}/en/card?id={{ $id }}</h2>
...
<h2>Une nouvelle <b>fiche de docteur</b> a été créée: {{ $root }}/fr/card?id={{ $id }}</h2>
